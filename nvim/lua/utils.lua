...
function M.is_plugin_installed(plugins_name)
    if fn.empty(fn.glob(M.plugins_path .. "/start/" .. plugins_name)) > 0 and
        fn.empty(fn.glob(M.plugins_path .. "/opt/" .. plugins_name)) > 0 then
        return false
    else
        return true
    end
...
function M.update_neovim()
    vim.notify("Updating NeoVim.", vim.log.levels.WARN, {
        title = "CodeArt"
    })
    if M.os == "Windows_NT" then
        vim.notify("Please check for NeoVim u.", vim.log.levels.WARN, {
            title = "CodeArt"
        })
    ...
    elseif M.os == "Linux" then
        ...
        vim.notify("NeoVim Updated.", vim.log.levels.WARN, {
            title = "CodeArt"
        })
    else
...
function M.update()
    ...
    if job_status == 0 then
        vim.api.nvim_notify("CodeArt Updated!", vim.log.levels.WARN, {
            title = "CodeArt"
        })
    else
        vim.notify("Update failed! Please try pulling manually.", vim.log.levels.ERROR, {
            title = "CodeArt"
        })
    end
end
